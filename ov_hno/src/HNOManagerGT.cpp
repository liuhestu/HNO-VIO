...
void HNOManager::process_camera_data(const ov_core::CameraData& msg) {
    ...
    // [Phase 1 严格模式]
    // 只有当获取到 GT 时，才允许 Update。这样能排除所有非系统本身原因导致的误差。
    if (!has_gt) {
        ROS_WARN_THROTTLE(1.0, "[Phase 1] No GT found for timestamp %.3f, skipping update step.", msg.timestamp);
        // 依然发布可视化和TF，但不进行 update
        publish_visualization(msg.timestamp, msg);
        publish_state(msg.timestamp, state); // Publish purely propagated state
        return; 
    }

    // 有 GT，传入 Feature 模块用于“完美建图”
    feature_handler->feed_measurement(
        msg, 
        state->R_hat_B2I, state->p_hat, // 估计值（用于RANSAC或原本逻辑）
        R_gt, p_gt,                     // 真值（用于建图和构建 p_world）
        valid_measurements
    );
    ...
    if(!valid_measurements.empty()) {
        ...
        ROS_INFO_THROTTLE(1.0, "[Phase 1] Updating with %d features (Map built with GT)", num_obs);
    } else {
        ROS_WARN_THROTTLE(1.0, "[Phase 1] Zero valid measurements!");
    }
...
void HNOManager::publish_visualization(double timestamp, const ov_core::CameraData& msg) {
    ...
    if(pub_img.getNumSubscribers() > 0) {
        cv::Mat img_viz;
        if(msg.images[0].channels() == 1)
            cv::cvtColor(msg.images[0], img_viz, cv::COLOR_GRAY2BGR);
        else
            msg.images[0].copyTo(img_viz);
...
void HNOManager::compute_and_print_error(double timestamp, const Eigen::Vector3d& p_est, int num_feats, int num_obs) {
    ...
    printf("[HNO] %.3f | Feat:%d/%d | Pos:%.2f %.2f %.2f | Vel:%.2f %.2f %.2f | Bg:%.4f %.4f %.4f | %s\n",
